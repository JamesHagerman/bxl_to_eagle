...
class HuffmanNode
  ...
  def to_binary_string
    if self.value.nil?
      ...
      bs = self.value.encode("ASCII-8BIT", {:invalid => :replace, :undef => :replace, :replace => '?'}).bytes.to_a[0].to_s(2)
      # bs = self.value.encode("UTF-8", {:invalid => :replace, :undef => :replace, :replace => '?'}).bytes.to_a[0].to_s(2)
